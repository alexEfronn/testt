...
class WheelController extends Controller {
	...
	public function newBet(Request $r) {
		...
		$color = $r->color;
		$balance = $r->balance;
		if(is_null($color)) return response()->json(['type' => 'error', 'msg' => 'Вы не выбрали множитель!']);
		if($color != 'black' && $color != 'red' && $color != 'green' && $color != 'yellow') return response()->json(['type' => 'error', 'msg' => 'Не удалось определить выбранный множитель!']);
		if($price < $this->settings->wheel_min_bet) return response()->json(['type' => 'error', 'msg' => 'Минимальная сумма ставки '.$this->settings->wheel_min_bet.'!']);
		if($price > $this->settings->wheel_max_bet) return response()->json(['type' => 'error', 'msg' => 'Максимальная сумма ставки '.$this->settings->wheel_max_bet.'!']);
		if($balance != 'balance' && $balance != 'bonus') return response()->json(['type' => 'error', 'msg' => 'Не удалось определить тип Вашего баланса!']);
		if($this->game->status > 1) return response()->json(['type' => 'error', 'msg' => 'Ставки в эту игру закрыты!']);
		if($balance == 'balance' && $this->user->balance < $price) return response()->json(['type' => 'error', 'msg' => 'Недостаточно баланса!']);
		if($balance == 'bonus' && $this->user->bonus < $price) return response()->json(['type' => 'error', 'msg' => 'Недостаточно баланса!']);
		...
		foreach($bets as $b) {
			if($balance != $b->balance) return response()->json(['type' => 'error', 'msg' => 'Вы уже сделали ставку с '. (($balance == 'balance') ? 'бонусного' : 'реального') .' счета!']);
			if($color != $b->color) return response()->json(['type' => 'error', 'msg' => 'Вы не можете сделать ставку на этот множитель!']);
		}
		...
		try {
			...
			DB::rollback();
			return response()->json(['type' => 'error', 'msg' => 'Неизвестная ошибка!']);
		}
		...
		return response()->json(['type' => 'success', 'msg' => 'Ваша ставка принята!']);
	}
	
	public function adminBet(Request $r) {
		$user = User::where('user_id', $r->user)->first();
		if(is_null($user)) return response()->json(['type' => 'error', 'msg' => 'Не удалось найти пользователя!']);
		$price = preg_replace('/[^0-9.]/', '', $r->sum);
		$color = $r->color;
		$balance = $r->balance;
		if(is_null($color)) return response()->json(['type' => 'error', 'msg' => 'Вы не выбрали множитель!']);
	...
	public function addBetFake() {
		if($this->game->status > 1) return [
            'success' => false,
            'fake' => $this->settings->fakebets,
            'msg' => '[Wheel] Ставки в эту игру закрыты!'
        ];
		$user = $this->getUser();
		if(!$user) return [
            'success' => false,
            'fake' => $this->settings->fakebets,
            'msg' => '[Wheel] Не удалось получить пользователя!'
        ];
		...
		if(is_null($color)) return [
            'success' => false,
            'fake' => $this->settings->fakebets,
            'msg' => '[Wheel] Вы не выбрали множитель!'
        ];
		if($color != 'black' && $color != 'red' && $color != 'green' && $color != 'yellow') return [
            'success' => false,
            'fake' => $this->settings->fakebets,
            'msg' => '[Wheel] Не удалось определить выбранный множитель!'
        ];
		if($price < $this->settings->wheel_min_bet) return [
            'success' => false,
            'fake' => $this->settings->fakebets,
            'msg' => '[Wheel] Минимальная сумма ставки '.$this->settings->wheel_min_bet.'!'
        ];
		if($price > $this->settings->wheel_max_bet) return [
            'success' => false,
            'fake' => $this->settings->fakebets,
            'msg' => '[Wheel] Максимальная сумма ставки '.$this->settings->wheel_max_bet.'!'
        ];
		if($balance != 'balance' && $balance != 'bonus') return [
            'success' => false,
            'fake' => $this->settings->fakebets,
            'msg' => '[Wheel] Не удалось определить тип Вашего баланса!'
        ];
		if($this->game->status > 1) return [
            'success' => false,
            'fake' => $this->settings->fakebets,
		...
		foreach($bets as $b) {
			if($balance != $b->balance) return [
				'success' => false,
				'fake' => $this->settings->fakebets,
				'msg' => '[Wheel] Вы уже сделали ставку с '. (($balance == 'balance') ? 'бонусного' : 'реального') .' счета!'
			];
			if($color != $b->color) return [
				'success' => false,
				'fake' => $this->settings->fakebets,
				'msg' => '[Wheel] Вы не можете сделать ставку на этот множитель!'
			];
		}
	...
	public function getSlider() {
		...
		$ranked = 0;
		$color = [];
		for($i = 0; $i < 40.9; $i++) $color[] = 'black';
		for($i = 0; $i < 32; $i++) $color[] = 'red';
		for($i = 0; $i < 27; $i++) $color[] = 'green';
