...
class DiceController extends Controller {
	...
	public function play(Request $r) {
		...
		if($sum < $this->settings->dice_min_bet) return response()->json(['type' => 'error', 'msg' => 'Минимальная сумма ставки '.$this->settings->dice_min_bet.' монет!']);
		if($sum > $this->settings->dice_max_bet) return response()->json(['type' => 'error', 'msg' => 'Максимальная сумма ставки '.$this->settings->dice_max_bet.' монет!']);
		if($sum > $this->user[$balType]) return response()->json(['type' => 'error', 'msg' => 'Вам не хватает монет для совершения ставки!']);
		if(!$perc) return response()->json(['type' => 'error', 'msg' => 'Вы не ввели шанс на победу!']);
		if(!$sum) return response()->json(['type' => 'error', 'msg' => 'Вы не ввели сумму ставки!']);
		if($perc < 1) return response()->json(['type' => 'error', 'msg' => 'Вы ввели неверный шанс!']);
		if($perc > 95) return response()->json(['type' => 'error', 'msg' => 'Вы ввели неверный шанс!']);
	...
	public function addBetFake() {
		...
		if($balType != 'balance' && $balType != 'bonus') return [
            'success' => false,
            'fake' => $this->settings->fakebets,
            'msg' => '[Dice] Не удалось определить тип Вашего баланса!'
        ];
		...
		try {
			if($sum < $this->settings->dice_min_bet) {
				...
				return [
					'success' => false,
					'fake' => $this->settings->fakebets,
					'msg' => '[Dice] Минимальная сумма ставки '.$this->settings->dice_min_bet.' монет!'
				];
			}
			if($sum > $this->settings->dice_max_bet) {
				...
				return [
					'success' => false,
					'fake' => $this->settings->fakebets,
					'msg' => '[Dice] Максимальная сумма ставки '.$this->settings->dice_max_bet.' монет!'
				];
			}
			if(!$perc) {
				DB::rollback();
	...
	public function adminBet(Request $r) {
		...
		try {
			if($sum < $this->settings->dice_min_bet) {
				DB::rollback();
				return response()->json(['type' => 'error', 'msg' => 'Минимальная сумма ставки '.$this->settings->dice_min_bet.' монет!']); 
			}
			if($sum > $this->settings->dice_max_bet) {
				DB::rollback();
				return response()->json(['type' => 'error', 'msg' => 'Максимальная сумма ставки '.$this->settings->dice_max_bet.' монет!']);
			}
			if(!$perc) {
				DB::rollback();
				return response()->json(['type' => 'error', 'msg' => 'Вы не ввели шанс на победу!']);
			}
			if(!$sum) {
				DB::rollback();
				return response()->json(['type' => 'error', 'msg' => 'Вы не ввели сумму ставки!']);
			}
			if($perc < 1) {
				DB::rollback();
				return response()->json(['type' => 'error', 'msg' => 'Вы ввели неверный шанс!']);
			}
			if($perc > 95) {
				DB::rollback();
				return response()->json(['type' => 'error', 'msg' => 'Вы ввели неверный шанс!']);
