...
class AdminController extends Controller {
	...
	public function addTask(Request $r) {
        \DB::table("tasks")->insert([
            "challengeId" => $r->get('challengeId'),
            "winnersCountType" => $r->get('countWinners'),
            "deposit" => $r->get('deposit'),
            "bank" => $r->get('bank'),
            "ended_at" => Carbon::now()->addDays($r->get('deadline'))->hour(explode(":", $r->get('endTime'))[0])->minute(explode(":", $r->get('endTime'))[1])
        ]);
        return redirect()->back()->with('success', 'Вы успешно добавили новое задание');
    }
    ...
    public function tasksAjax() {
        return datatables(\DB::select('SELECT tasks.*, (SELECT COUNT(*) FROM tasks_users WHERE tId = tasks.id) as count FROM tasks'))->toJson();
    }
    ...
    $sends_arr_from = Sends::where('receiver', $user->id)->get();
		...
		return view('admin.user', compact('user', 'pay', 'withdraw', 'exchanges', 'jackpotWin', 'jackpotLose','sends', 'sends_from', 'wheelWin', 'wheelLose', 'crashWin', 'crashLose', 'coinWin', 'coinLose', 'battleWin', 'battleLose', 'diceWin', 'diceLose', 'betWin', 'betLose'));
    }
	...
	public function userSave(Request $r) {
        ...
        if($r->get('bonus') == null) return back()->with('error', 'Поле "Бонус" не может быть пустым!');
    ...
    public function fakeSave(Request $r) {
        ...
        return redirect()->route('admin.bots')->with('success', 'Бот добавлен!');
    }
	...
	public function botsDelete($id) {
		...
		return redirect()->route('admin.bots')->with('success', 'Бот удален!');
    }
    ...
    public function bonusNew(Request $r) {
        ...
        $color = $r->get('color');
        $status = $r->get('status');
        if(!$sum) return redirect()->route('admin.bonus')->with('error', 'Поле "Сумма" не может быть пустым!');
        if(!$bg) return redirect()->route('admin.bonus')->with('error', 'Поле "Цвет фона" не может быть пустым!');
        if(!$color) return redirect()->route('admin.bonus')->with('error', 'Поле "Цвет текста" не может быть пустым!');
        ...
        return redirect()->route('admin.bonus')->with('success', 'Бонус создан!');
    }
    ...
    public function bonusSave(Request $r) {
        ...
        $color = $r->get('color');
        $status = $r->get('status');
        if(!$id) return redirect()->route('admin.bonus')->with('error', 'Не удалось найти данный ID!');
        if(!$sum) return redirect()->route('admin.bonus')->with('error', 'Поле "Сумма" не может быть пустым!');
        if(!$bg) return redirect()->route('admin.bonus')->with('error', 'Поле "Цвет фона" не может быть пустым!');
        if(!$color) return redirect()->route('admin.bonus')->with('error', 'Поле "Цвет текста" не может быть пустым!');
        ...
        return redirect()->route('admin.bonus')->with('success', 'Бонус обновлен!');
    }

    public function bonusDelete($id) {
        if(!$id) return redirect()->route('admin.bonus')->with('error', 'Нет такого бонуса!');
        Bonus::where('id', $id)->delete();

        return redirect()->route('admin.bonus')->with('success', 'Бонус удален!');
    }
    ...
    public function giveawayNew(Request $r) {
        ...
        if(!$sum) return redirect()->route('admin.giveaway')->with('error', 'Поле "Сумма" не может быть пустым!');
        if(!$time_to) return redirect()->route('admin.giveaway')->with('error', 'Поле "Время окончания" не может быть пустым!');
        ...
        return redirect()->route('admin.giveaway')->with('success', 'Раздача создана!');
    }
    ...
    public function giveawaySave(Request $r) {
        ...
        if(!$id) return redirect()->route('admin.giveaway')->with('error', 'Не удалось найти данный ID!');
        if(!$sum) return redirect()->route('admin.giveaway')->with('error', 'Поле "Сумма" не может быть пустым!');
        if(!$time_to) return redirect()->route('admin.giveaway')->with('error', 'Поле "Время окончания" не может быть пустым!');
        ...
        return redirect()->route('admin.giveaway')->with('success', 'Раздача обновлена!');
    }

    public function giveawayDelete($id) {
        if(!$id) return redirect()->route('admin.giveaway')->with('error', 'Нет такой раздачи!');
        ...
        return redirect()->route('admin.giveaway')->with('success', 'Раздача удалена!');
    }
    ...
    public function promoNew(Request $r) {
        ...
        if(!$amount) return redirect()->route('admin.promo')->with('error', 'Поле "Сумма" не может быть пустым!');
    ...
    public function filterNew(Request $r) {
        ...
        if(!$word) return redirect()->route('admin.filter')->with('error', 'Поле "Фильтр" не может быть пустым!');
        if($have) return redirect()->route('admin.filter')->with('error', 'Такой фильтр уже существует');

        Filter::create([
            'word' => $word
        ]);

        return redirect()->route('admin.filter')->with('success', 'Фильтр создан!');
    }

    public function filterSave(Request $r) {
        $word = $r->get('word');
        $have = Filter::where('word', $word)->first();
        if(!$id) return redirect()->route('admin.filter')->with('error', 'Не удалось найти данный ID!');
        if(!$word) return redirect()->route('admin.filter')->with('error', 'Поле "Фильтр" не может быть пустым!');
        if($have) return redirect()->route('admin.filter')->with('error', 'Такой фильтр уже существует');
        ...
        return redirect()->route('admin.filter')->with('success', 'Фильтр обновлен!');
    }

    public function filterDelete($id) {
        if(!$id) return redirect()->route('admin.filter')->with('error', 'Нет такого фильтра!');
        Filter::where('id', $id)->delete();

        return redirect()->route('admin.filter')->with('success', 'Фильтр удален!');
    }
	...
	public function withdrawSend($id) {
		...
		if($withdraw->status > 0) return redirect()->route('admin.withdraws')->with('error', 'Этот вывод уже обработан или отменен');

		if($withdraw->system == 'payeer') {
			$code = null;
			...
			if(isset($response['errors']) && ! empty($response['errors'])) {
				...
				if($mes == 'balanceError') return redirect()->route('admin.withdraws')->with('error', 'На кошельке Payeer недостаточно средств для вывода средств!');
				if($mes == 'transferError') return redirect()->route('admin.withdraws')->with('error', 'Ошибка перевода, необходимо повторить перевод через некоторое время!');
				if($mes == 'You do not have sufficient funds on your account.') return redirect()->route('admin.withdraws')->with('error', 'На кошельке Payeer недостаточно средств для вывода средств!');
		...
		if($withdraw->system == 'yandex' || $withdraw->system == 'webmoney' || $withdraw->system == 'visa' || $withdraw->system == 'qiwi') {
			if($withdraw->system == 'qiwi') $code = 63;
			if($withdraw->system == 'yandex') $code = 45;
			if($withdraw->system == 'webmoney') $code = 1;
			...
			if($result['status'] == 'error') {
				if($result['desc'] == 'Balance too low') $desc = 'На кошельке Free-Kassa недостаточно средств для вывода средств!';
				if($result['desc'] == 'Cant make payment to anonymous wallets') $desc = 'Данный пользователь использует анонимный кошелек!.';
				if($result['desc'] == 'РЎР»РёС€РєРѕРј С‡Р°СЃС‚С‹Рµ Р·Р°РїСЂРѕСЃС‹ Рє API') $desc = 'Неизвестная ошибка!.';
		...
		return redirect()->route('admin.withdraws')->with('success', 'Вывод поставлен в очередь!');
	}
    ...
    public function withdrawReturn($id) {
        ...
        return redirect()->route('admin.withdraws')->with('success', 'Вы вернули '.$withdraw->valueWithCom.'р. на баланс '.$user->username);
    }
	...
	public function getBanned() {
		...
		if(is_null($users)) return response()->json(['success' => false, 'msg' => 'Не удалось найти пользователей!', 'type' => 'error']);
	...
	public function getBalancePE() {
        ...
        return $response['balance']['RUB']['BUDGET'];
    }
