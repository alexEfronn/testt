...
class CrashController extends Controller {
    ...
    public function newBet(Request $r) {
        ...
        if(floatval($r->get('withdraw')) > 10000 && floatval($r->get('withdraw')) < 0) return [
            'success' => false,
            'msg' => 'Вы ввели не верное значение для автовывода!'
        ];
    ...
    public function addBetFake() {
        ...
        if(floatval($sum) < $this->settings->crash_min_bet) return [
            'success' => false,
            'fake' => $this->settings->fake,
            'msg' => '[Crash] Минимальная сумма ставки - ' . $this->settings->crash_min_bet
        ];

        if($this->settings->crash_max_bet > 0 && $this->settings->crash_max_bet < floatval($sum)) return [
            'success' => false,
            'fake' => $this->settings->fake,
            'msg' => '[Crash] Максимальная сумма ставки - ' . $this->settings->crash_max_bet
        ];
    ...
    public function getFloat() {
        ...
        if(is_null($lastZero) || ($this->game->id-$lastZero->id) >= mt_rand(2, 5)) return 1;
        ...
        if($m == 1) return $list[0].'.0'.mt_rand(0,9);
    ...
    public function Cashout() {
        ...
        try {
            ...
            return [
                'success' => true,
                'msg' => 'Поздравляем! Вы умножили свою ставку на x'. $float .' и получили '. round($bet->price*$float, 2) .' монет!'
            ];
