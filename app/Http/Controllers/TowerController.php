...
class TowerController extends Controller
{
    ...
    public function newGame(Request $r)
    {
		...
		if($bet < $this->settings->tower_min_bet) return response()->json(['success' => false, 'message' => 'Минимальная сумма ставки '.$this->settings->tower_min_bet.'!']);
		if($bet > $this->settings->tower_max_bet) return response()->json(['success' => false, 'message' => 'Максимальная сумма ставки '.$this->settings->tower_max_bet.'!']);
		if ($balType !== 'balance' && $balType !== 'bonus') return ['success' => false, 'message' => 'Не удалось определить тип Вашего баланса!'];
		if ($bombs < 1 || $bombs > 4) return ['success' => false, 'message' => 'Не удалось определить количество бомб!'];
		if (!$bet) return ['success' => false, 'message' => 'Вы не ввели сумму ставки!'];
		if (!$bombs) return ['success' => false, 'message' => 'Вы не указали количество бомб'];
		if ($this->user[$balType] < $bet) return ['success' => false, 'message' => 'Недостаточно средств на балансе'];
		$lastGame = Tower::where('user_id', $this->user->id)->where('status', 0)->first();
		if ($lastGame) return ['success' => false, 'message' => 'Закончите прошлую игру'];
    ...
    public function next(Request $r)
    {
		...
		$game = Tower::where('user_id', $this->user->id)->where('status', 0)->first();
		if (!$game) return ['success' => false, 'message' => 'Произошла ошибка, обновите страницу'];

		$slot = intval($r->slot);
		if ($slot < 0 || $slot > 4) return ['success' => false, 'message' => 'Произошла ошибка, обновите страницу'];
    ...
    public function claim()
    {
		...
		$revealed = json_decode($game->revealed);
		if (count($revealed) === 0) return ['success' => false, 'message' => 'Откройте 1 ячейку'];
