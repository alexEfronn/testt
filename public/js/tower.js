...
class Tower {
	...
	currentStep() {
		...
		$('.claim').html(`Забрать ${this.game.state.claim}`);
		...
		if (this.game.currency === 'bonus') {
			$('.tower_component__3oM-1').append(`<div class="game-tooltip isTransparent isActive demo won">`+
							`<div class="wrap">`+
								...
								`</div>`+
								`<div class="status">Вы выиграли<span class="profit">${(this.game.bet * this.game.coeff).toFixed(2)}</span>`+
								`</div>`+
								`<button type="button" class="btn btn-change">Играть на деньги</button>`+
							`</div>`+
						`</div>`);
		} else {
