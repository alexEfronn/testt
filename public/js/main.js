...
$(document).ready(function() {
	...
	$('#settings-change-theme').click(function() {
		$.ajax({
            ...
            success : function(data) {
				...
				if(data == 'dark') {
					$('#settings-change-theme').attr('checked', true);
					$('#css_theme').attr('href', '/css/main_dark.css?v=7');
				} else {
					$('#settings-change-theme').attr('checked', false);
					$('#css_theme').attr('href', '/css/main.css?v=7');
				}
            },
            error : function(data) {
                console.log(data.responseText);
	...
	/*Промо в чат*/
$(document).on('click', '.activateChatPromo', function() {
		var promo = $('#promoChatInput').val();
		$.ajax({
			url: '/chat',
			type: 'post',
			data: {
...
/***********/
	...
	.on('chat', function (data) {
		...
		$('.chat-conversation').append(
			'<a href="#chatm_'+ data.time2 +'" class="scrollTop"><i class="fa fa-angle-double-down"></i></a>'
			);*/
		/***********/
   ...
   .on("quiz.winner", (data) => {
			...
			<div class="message slide-out">
				<div class="MessageQuiz_message__1wS5q">
					<div class="MessageQuiz_caption__iD8Nv">
						<span class="MessageQuiz_captionLabel__YsT5P">Викторина!</span>
					</div>
					<div class="MessageQuiz_desc__134DU"><b style="color: #008ffb;">${data.user.username}</b> первым дал правильный ответ на вопрос викторины, и он получает <div class="bet-number">
							<span class="bet-wrap">
								<span style="color: ${data.type == "balance" ? "#00a0e3;" : "#ffc645;"}" class="won">${data.amount.toFixed(2)}</span>
								<svg style="fill: ${data.type == "balance" ? "#00a0e3;" : "#ffc645;"}" class="icon icon-coin">
	...
	.on("quiz.new", (data) => {
				...
				<div class="MessageQuiz_message__1wS5q">
					...
					<div class="MessageQuiz_desc__134DU">
						...
						<div class="bet-number">
							<span class="bet-wrap">
								<span style="color: ${data.typeBalance == "balance" ? "#00a0e3;" : "#ffc645;"}" class="won">${data.amount.toFixed(2)}</span>
								<svg style="fill: ${data.typeBalance == "balance" ? "#00a0e3;" : "#ffc645;"}" class="icon icon-coin">
									<use xlink:href="/img/symbols.svg#icon-coin"></use>
								</svg>
							</span>
						</div>
		...
		`);
        if($('.chat_blok .message').length >= 20) $('.chat_blok .message:nth-child(1)').remove();
		container.scrollTop = container.scrollHeight - container.clientHeight;
		ps.update();
	})
