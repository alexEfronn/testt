...
class JackpotController extends Controller {
	...
	public function newBet(Request $r) {
		...
		if($game->status > 1) return [
			'success' => false,
			'msg' => 'Ставки в этой игре закрыты!'
		];
		...
		if($userbets->count() >= $room->bets) return [
			'success' => false,
			'msg' => 'Вы не можете сделать больше '.$room->bets.' ставок за одну игру!'
		];
			
		if(floatval($r->get('amount')) < $room->min) return [
			'success' => false,
			'msg' => 'Минимальная сумма ставки '.$room->min.' монет(а/ы)!'
		];

		if(floatval($r->get('amount')) > ($room->max - $userbets->sum('sum'))) return [
			'success' => false,
			'msg' => 'Вы не можете сделать ставку больше '.$room->max.' монет(а/ы) за одну игру!'
		];
		...
		try {
			...
			if($betType > 0) {
				...
				return [
					'success' => false,
					'msg' => 'Вы уже сделали ставку с '. (($balance == 'balance') ? 'бонусного' : 'реального') .' счета!'
				];
			}
			if($balance != 'balance' && $balance != 'bonus') {
				...
				return [
					'success' => false,
					'msg' => 'Не удалось определить тип Вашего баланса!'
				];
			}
	...
	public function addBetFake() {
		...
		if(is_null($room)) return [
			...
			'msg' => '[ROOM #'.$room->name.'] Не удалось найти комнату, в которой вы хотите сделать ставку!'
		];
		if(is_null($user)) return [
			'success' => false,
            'fake' => $this->settings->fakebets,
			'msg' => '[ROOM #'.$room->name.'] Не удалось найти пользователя!'
		];
		if(is_null($game)) return [
			'success' => false,
            'fake' => $this->settings->fakebets,
			'msg' => '[ROOM #'.$room->name.'] Не удалось найти игру!'
		];

		if($game->status > 1) return [
			'success' => false,
            'fake' => $this->settings->fakebets,
			'msg' => '[ROOM #'.$room->name.'] Ставки в этой игре закрыты!'
		];
		...
		try {
			...
			if($userbets->count() >= $room->bets) {
				...
				return [
					'success' => false,
            		'fake' => $this->settings->fakebets,
					'msg' => '[ROOM #'.$room->name.'] Вы не можете сделать больше '.$room->bets.' ставок за одну игру!'
				];
			}
			...
			if(floatval($sum) < $room->min) {
				...
				return [
					'success' => false,
            		'fake' => $this->settings->fakebets,
					'msg' => '[ROOM #'.$room->name.'] Минимальная сумма ставки '.$room->min.' монет(а/ы)!'
				];
			}
			...
			if(floatval($sum) > ($room->max - $userbets->sum('sum'))) {
				...
				return [
					'success' => false,
            		'fake' => $this->settings->fakebets,
					'msg' => '[ROOM #'.$room->name.'] Вы не можете сделать ставку больше '.$room->max.' монет(а/ы) за одну игру!'
				];
			}
			...
			if(is_null($user)) {
				...
				return [
					'success' => false,
            		'fake' => $this->settings->fakebets,
					'msg' => '[ROOM #'.$room->name.'] Неизвестная ошибка!'
				];
			}
			...
			if($betType > 0) {
				...
				return [
					'success' => false,
            		'fake' => $this->settings->fakebets,
					'msg' => '[ROOM #'.$room->name.'] Вы уже сделали ставку с '. (($balance == 'balance') ? 'бонусного' : 'реального') .' счета!'
				];
			}
			if($balance != 'balance' && $balance != 'bonus') {
				DB::rollback();
				return [
					'success' => false,
            		'fake' => $this->settings->fakebets,
					'msg' => '[ROOM #'.$room->name.'] Не удалось определить тип Вашего баланса!'
				];
			}
		...
		return [
			'success' => true,
			'fake' => $this->settings->fakebets,
			'msg' => '[ROOM #'.$room->name.'] Ваша ставка Вошла в игру!'
		];
	}
	...
	public function getSlider(Request $r) {
		...
		if(!$room) return [
			'success' => false,
			'msg' => 'Не удалось найти комнату '.$r->get('room')
		];
		...
		if($game->id != $r->get('game')) return [
			'success' => false,
			'msg' => 'Найдена игра #'.$game->id.'. Не является #'.$r->get('game')
		];
