...
<body> 
<input type="hidden" value="{{ session('sound') }}" id="soundController">
<input type="hidden" value="{{ session('theme') }}" id="themeController">
<audio id="startthisgame" src=/sounds/startthisgame.mp3 preload=auto></audio>
	<audio id="common-bet" src=/sounds/common-bet.mp3 preload=auto></audio>
    <div class="wrapper">
        <div class="page">
                ...
                <ul class="left-sidebar__games">
                   <li class="{{ Request::is('/jackpot') ? 'current' : '' || Request::is('jackpot/history') ? 'current' : ''  || Request::is('jackpot/history/*') ? 'current' : '' }}">
                    ...
                    <li class="{{ Request::is('crash') ? 'current' : '' }}">
					 ...
					 <li class="{{ Request::is('cases') ? 'current' : '' }}">
                        <a class="" href="/wheel">
                    ...
                    <div class="footer">
                        <div class="container">
                            <div class="row">
                                ...
                                <div class="col col-5">
                                    <div class="copyright">
                                        <div class="text">© 2021 {{$settings->sitename}}
                                        ...
                                        <div>
                                            <button class="user-link" type="button" data-id="{{$sms['unique_id']}}">
                                                <span class="sanitize-name">
                                                	<span class="sanitize-text">@if($sms['admin'])<span class="admin-badge isAdmin" data-toggle="tooltip" data-placement="top" title="Администратор"><span class=""><svg class="icon iconon-a"><use xlink:href="/img/symbols.svg#icon-a"></use></svg></span></span> Администратор @elseif($sms['moder'])<span class="admin-badge isModerator" data-toggle="tooltip" data-placement="top" title="Модератор"><span class=""><svg class="icon icon-m"><use xlink:href="/img/symbols.svg#icon-m"></use></svg> 
                                                    {{$sms['username']}} @elseif($sms['youtuber'])<span class="admin-badge isYouTuber" data-toggle="tooltip" data-placement="top" title="YouTuber"><span class=""><svg class="icon icon-y"><use xlink:href="/img/symbols.svg#icon-y"></use></svg></span></span> 
                                                    {{$sms['username']}} @else {{$sms['username']}}
							...
							<div class="user-avatar">
                                ...
                                <div class="give-block">
                                    <a href="/profile" class="btn-profile" style="align: center;">
                                        <span>
                                            <img src="{{$u->avatar}}" style="width: 75px;height: 75px;float: left;border-radius: 50%;    border: 1px solid #2cb6cd;">
                                            <p style="margin-left: 13px;float: left;line-height: 75px;font-size: 16 px;">{{$u->username}}</p>
                                        </span>
                                    </a>
                                </div>
							</div>
					...
					<div class="wrap">
						...
						<div class="deposit-section tab active" data-type="deposite">
							<form action="/pay" method="post" id="payment">
								...
								<div class="form-row">
									<label>
										<div class="form-label big">Пополняй счет на сумму от {{$settings->dep_bonus_min}}р. и получай <center> бонус +{{$settings->dep_bonus_perc}}% на свой баланс!</center></div>
									</label>
								</div>
								@endif
								<div class="form-row">
						...
						<div class="faucet-modal-give {{$gv->status == 1 ? 'doneGive' : ''}}" id="gv_{{$gv->id}}">
							<div class="give-btn-block">
								<div class="faucet-reload">
									<h3><div class="faucet-cd">Раздача #{{$gv->id}}</div></h3>
								</div>
							</div>
							<br>
							<div class="row">
								<div class="col-6">
									<div class="bank-text">Сумма: {{$gv->sum}} <svg class="icon icon-coin {{$gv->type}}"><use xlink:href="/img/symbols.svg#icon-coin"></use></svg></div>
								</div>
								<div class="col-6">
									<div class="date-text">До начала: {{$gv->status == 0 ? $gv->time_to : 'Проведена'}}</div>
								</div>
							</div>
							@if($gv->status == 0)
	...
	<div class="modal fade" id="captchaModal" tabindex="-1" role="dialog" aria-labelledby="captchaModalLabel" aria-hidden="true">
	 ...
	 <!--Промокод в чат-->@if(Auth::user() and $u->is_admin == 1 || $u->is_moder == 1)
